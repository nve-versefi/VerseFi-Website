        ...
        <div className="container">
          <div className="relative -mx-4 flex items-center justify-between">
            <div className="w-60 max-w-full px-4 xl:mr-12">
              <Link href="/"className={`header-logo block w-full ${sticky ? "py-0 lg:py-0" : "py-0"}`}>
                  ...
                  <ul className="block lg:flex lg:space-x-12">
                    {menuData.map((menuItem, index) => (
                      <li key={menuItem.id} className="group relative">
                        {menuItem.path ? (
                          <Link href={menuItem.path} className="flex py-2 text-xl font-bold text-blueribbon-500 group-hover:opacity-70 dark:text-blueribbon-500 lg:mr-0 lg:inline-flex lg:py-6 lg:px-0 mr-4 lg:mr-6">
                          ...
                          <>
                            <div onClick={() => handleSubmenu(index)} className="flex cursor-pointer items-center justify-between py-2 text-xl font-bold text-blueribbon-500 group-hover:opacity-70 dark:text-blueribbon-500 lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">
                              {menuItem.title}
                              <span className="pl-3">
                            ...
                            <div className={`submenu relative top-full left-0 rounded-md bg-white transition-[top] duration-300 group-hover:opacity-100 dark:bg-dark lg:invisible lg:absolute lg:top-[110%] lg:block lg:w-[250px] lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full ${openIndex === index ? "block" : "hidden"}`}>
                              {menuItem.submenu && menuItem.submenu.map((submenuItem) => (
                                <Link key={submenuItem.id} href={submenuItem.path} className="block rounded py-2.5 text-sm text-dark text-blueribbon-500 hover:opacity-70 dark:text-white lg:px-3">
